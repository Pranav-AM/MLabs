...
def generate_person_object_locations_prevframes(sequence):
	...
	#print 'persons length: ', len(persons)
...
def system_worker():
    ...
    object_detection_path = OBJECT_PATH
    pose_estimation_path = POSE_PATH
    ...
    num_frames = len([1 for name in os.listdir(frame_path) if os.path.isfile(frame_path+name)])
    #print(num_frames)
    while(1):
        #print(cur_frame_number)
	if(cur_frame_number > num_frames-1):
	    print("Frames over")
	    break
		...
		print("File not found: ", object_filepath)
		MISSING_FILES.append(object_filepath)

 	    try:
		person_read = open(person_filepath, 'r')
            except:
                print("File not found", person_filepath)
		MISSING_FILES.append(person_filepath)
 		#print(object_filepath)
                #print(person_filepath)
                inp = 1 #int(input("do you want to continue 1 continue and 0 break?"))
                if inp == 0:
                    break_flag = 1
                    break
	...
	print 'Current frame: ' , cur_frame_number
...
#            cv2.putText(cur_img, text, (10 * count, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
        ...
        img_name = IMAGE_PATH + str(cur_frame_number) + ".jpg"
...
#        cur_frame_number += 1
    ...
    ##############################################################################################
    out = cv2.VideoWriter('../data/activity_video_2.avi',cv2.cv.CV_FOURCC(*'XVID'), 10, size) #cv2.VideoWriter_fourcc(*'DIVX'), 10, size)
    ##############################################################################################
    ...
    #print 'These files were not found: ', missing_files
