...
def joints(oriImg,pose,imname):
    ...
    vel=[]
    if len(data)==0:
        for veli in range(18):
            vel.append(0)
    else:
        for veli in range(18):
            x2=joint_peaks[veli][0]
            y2=joint_peaks[veli][1]
            x1=data[-1][1][veli][0]
            y1=data[-1][1][veli][1]
            vel.append([(x2-x1)/0.1,(y2-y1)/0.1])
    ...
    for i in range(17):
        for n in range(len(subset)):
            ...
            l=[[X[0],X[1]],[Y[0],Y[1]],length,angle]
            limbs.append(l)
    ...
    nl=[imname,joint_peaks,limbs,vel,pose]
    toc =time.time()
    time_taken=toc-tic   
    cv2.imwrite('./newframes/'+imname+'.jpg',canvas)
...
for file in sorted(os.listdir('./videos')):
    ...
    if file[0:1] == 'a':
        ...
        pose='background'
    #single=[]
    for image in sorted(os.listdir('./videos/'+file)):
        test_image = './videos/'+file+'/'+image
        oriImg = cv2.imread(test_image) # B,G,R order
        try:
            imname=file+'_'+image
            nl,time_taken=joints(oriImg,pose,imname)
            data.append(nl)
            #track.append([counter,imname,time_taken])
            #single.append(nl[3])
            print imname,time_taken
            #print nl
        except Exception,e:
            print 'error image'
            print str(e)
        counter=counter+1
    #print data.shape
    #velocities.append([pose,single])
    #import pickle
    pickle.dump( data, open( "mini_gen_data"+file+".p", "wb" ) )
    #totaldata.append(data)
#print totaldata.shape,track.shape
#import pickle
#pickle.dump( totaldata, open( "gen_data.p", "wb" ) )
#pickle.dump( track, open( "gen_track.p", "wb" ) )
#pickle.dump( velocities, open( "gen_vel.p", "wb" ) )
